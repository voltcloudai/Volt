...
fn prompt_language_rules() -> Vec<&'static str> {
    vec![
        ...
        "Keep inline route bodies inside the Axum lowering subset: const bindings and `return ok(...)` over literals, field access, array indexing, `array.length`, simple calls, and struct literals.",
        "Do not use null.",
...
fn steps_for(module: &str, action: &str, method: &str) -> Vec<String> {
    ...
    vec![
        ...
        "Add focused validation coverage using currently supported Volt functions or compiler tests.".to_string(),
